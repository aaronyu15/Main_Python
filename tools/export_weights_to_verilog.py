...
"""Export EventSNNFlowNetLite weights for SystemVerilog.
...
coarser approximation than the Python per-output-channel quantizer.
"""
...
def _compute_decay_q(decay: float, mem_q: int) -> int:
	"""Return fixed-point decay coefficient in Qmem_q."""
	decay_f = float(decay)
	if not math.isfinite(decay_f):
		decay_f = 0.0
	decay_f = max(0.0, min(1.0, decay_f))
	return int(round(decay_f * (2**mem_q)))
...
def main() -> None:
	...
	# Note: EventSNNFlowNetLite in this repo uses add-based skips (no align convs).
	convs = {
	...
	for name, conv in convs.items():
		...
		# For spiking conv blocks, inputs are small integers (spike counts), so we
		# want an output in Q(MEM_Q): cur_q = (acc << MEM_Q) >> (MEM_Q + s) == acc >> s.
		# We keep the convention LAYER_SHIFT = MEM_Q + s for those layers.
		#
		# For the flow head, the input is membrane in Q(MEM_Q) already.
		# acc is therefore already in Q(MEM_Q), and we only need to apply the weight
		# scale shift: flow_q = acc >> s.
		if name == "flow":
			layer_shift = shift_s
		else:
		...
		if conv.bias is not None and name in {"e1", "e2", "e3", "d3", "d2", "d1", "flow"}:
	...
	decay_q = _compute_decay_q(decay, mem_q)
	thresh_q = _compute_thresh_q(threshold, mem_q)
	# Optional integer flow scaling (implemented as << FLOW_SCALE_POW2 in RTL).
	flow_scale_pow2 = int(cfg.get("flow_scale_pow2", 0)) if "flow_scale_pow2" in cfg else 0
	...
	for name in ("e1", "e2", "e3", "d3", "d2", "d1", "flow"):
