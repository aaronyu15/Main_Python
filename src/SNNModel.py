...
def build_layer_arrays(N_prev, N):
    tau         = np.full(N, 1.0, dtype=np.float32)
    v_th        = np.full(N, 0.9, dtype=np.float32)
    neuron_id   = np.random.randint(0, 16, size=N, dtype=np.int64)
    t_last      = np.int64(0)

    membrane_potential_fict = np.zeros(N, dtype=np.float32)
    membrane_potential_phys = np.zeros(N, dtype=np.int64)
    membrane_potential_map  = 1.0 / (1.0 + np.exp(-(np.random.random((N, 16)).astype(np.float32) * 5.0)))
    synapse_w_matrix        = np.random.random((N_prev, N)).astype(np.float32)
...
def forward_batch_numba(
    ...
    t_last_list,                        # Local state (mutable)
    ...
    for b in prange(B):
        ...
        for l in range(num_layers):
            local_t_last.append(t_last_list[l])  # scalar copy
    ...
    return spike_neurons, spike_times, spike_counts
...
class model:
    ...
    def __call__(self, frames_batch):
        ...
        spike_neurons, spike_times, spike_counts = forward_batch_numba(
            frames_batch,
            self.X, 
            self.Y, 
            self.P,
